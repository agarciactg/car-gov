...
from car.apps.base import models
...
@register(models.Government)
class GovernmentAdmin(ModelAdmin):
    list_display = ("id", "name")
    search_fields = ("name",)
    ordering = ("name",)
    icon_name = "book"
...
@register(models.EnvironmentalAuthorities)
class EnvironmentalAuthoritiesAdmin(ModelAdmin):
    list_display = ("id", "name")
    search_fields = ("name",)
    ordering = ("name",)
    icon_name = "assistant_photo"
    list_per_page = settings.NUMBER_PAGINATION_ADMIN
